...
describe('GET /study/writing_answers', function() {
    ...
    describe('response.failure', function() {
        it(`should respond with 401 Unauthorized if the request does not contain an 'authorization' header`, function(done) {
            request(server).get('/study/writing_answers/en').expect(401, done);
        });
        ...
        it(`should return a 400 response if the 'since' query parameter value is a future date`, function(done) {
            ...
            request(server).get(`/study/writing_answers/en?since=${twoDaysLater}`).set('authorization', authorization).expect(400, done);
        });

        it(`should return 400 Bad Request if the 'lang' parameter is not a valid language code`, function(done) {
            request(server).get(`/study/writing_answers/invalid`).set('authorization', authorization).expect(400, done);
        });

        it(`should return a 400 response if the 'max_id' query param value is 0`, function(done) {
            request(server).get('/study/writing_answers/en?max_id=0').set('authorization', authorization).expect(400, done);
        });

        it(`should return a 400 response if the 'max_id' query param value is a negative number`, function(done) {
            request(server).get('/study/writing_answers/en?max_id=-1000').set('authorization', authorization).expect(400, done);
        });
    });

    describe('response.success', function() {
        it('should receive a 200 OK response', function(done) {
            request(server).get('/study/writing_answers/en').set('authorization', authorization).expect(200, done);
        });
        ...
        it(`should have an non-null 'lang' language code for each record that is equal to the 'lang' request parameter value`, function() {
            const langCode = 'en';
            return request(server).get(`/study/writing_answers/${langCode}`).set('authorization', authorization).then(function(response) {
        ...
        it(`should apply the timezone offset in the request to 'created_at'`, function() {
            return request(server).get(`/study/writing_answers/en?time_zone_offset=-540`).set('authorization', authorization).then(function(response) {
        ...
        it(`should respond with records whose creation date is equal to or greater than the 'since' query parameter`, function() {
            ...
            return request(server).get(`/study/writing_answers/en?since=${thirtyDaysAgo}`).set('authorization', authorization).then(function(response) {
        ...
        it(`should return only records whose IDs are less than or equal to the 'max_id' query parameter`, function() {
            ...
            return db.sequelize.query(allUserWritingAnswers).then(function(answers) {
                ...
                return request(server).get(`/study/writing_answers/en?max_id=${midWritingAnswerId}`).set('authorization', authorization).then(function(response) {
        ...
        it(`should return only records whose language is equal to that of the 'lang' request param`, function() {
            ...
            return request(server).get(`/study/writing_answers/${requestLang}`).set('authorization', authorization).then(function(response) {
